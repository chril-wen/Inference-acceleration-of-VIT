...
class ViT(nn.Module):
    def __init__(self,emb_size=16):
        ...
        self.encoder=Encoder(emb_size=emb_size,q_k_size=512,v_size=256,f_size=2048,head=16,nblocks=2048,dropout=0.1)
        ...
        self.cls_linear=nn.Linear(in_features=emb_size,out_features=9) # 9种分类
        
    def forward(self,x): # (batch_size,channel=3,width=28,height=28)
        ...
        y=self.encoder(x) # 不涉及padding，所以不需要mask   (batch_size,50,emb_size)
